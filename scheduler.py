...
from mirror import RepositoryClasses
import time
    ...
    if tasks and tasks.count() > 0:
        for task in tasks:
            if not task.repository.schedule_run:
                threadList[task.repository.name] = ScheduleTaskRunner(
                    RepositoryClasses.RepositoryUpdate(task.repository)).run(task.get_id())
